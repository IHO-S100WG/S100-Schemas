...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron">
  ...
  -->
  <!--
    Updated by Raphael Malyankar for use with S-100 4.0.0 metadata by:
    (1) updating version numbers in selected namespaces to match the namespaces
        used by S-100 4.0.0 (the .sch files for the 2.0 versions are not on the
        ISO standards.iso.org web site or GitHub distribution as of April 22, 2019).
    (2) Add role attributes to prevent informative diagnostics from raising error flags.
  -->
    ...
    <sch:rule context="/">
      ...
      <!--<sch:report test="$hasOneMD_MetadataElement"
        diagnostics="rule.mdb.root-element-success-en 
                     rule.mdb.root-element-success-fr"  role="info"/>-->
    </sch:rule>
  </sch:pattern>
    ...
    <sch:rule context="/mdb:MD_Metadata/mdb:defaultLocale|
      ...
      <!--<sch:report test="$hasEncoding"
        diagnostics="rule.mdb.defaultlocale-success-en
                     rule.mdb.defaultlocale-success-fr" role="info"/>-->
    </sch:rule>
  </sch:pattern>
    ...
    <sch:rule context="/mdb:MD_Metadata/mdb:metadataScope/
      ...
      <sch:report test="$hasScopeCodeName or $hasNilReason"
        diagnostics="rule.mdb.scope-name-success-en
                     rule.mdb.scope-name-success-fr" role="info"/>
    </sch:rule>
  </sch:pattern>
    ...
    <sch:rule context="mdb:MD_Metadata">
      ...
      <!--<sch:report test="$hasAtLeastOneCreationDate"
        diagnostics="rule.mdb.create-date-success-en
                     rule.mdb.create-date-success-fr" role="info"/>-->
    </sch:rule>
  </sch:pattern>
</sch:schema>
